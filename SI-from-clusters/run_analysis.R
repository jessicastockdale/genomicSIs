...
dir.create(file.path("../Figures"), showWarnings = FALSE)
 ...
 ## Regular (non-pooled) tree version
  ...
  for (i in 1:length(results_sam1)){
    ...
    # Tree sampling, choice of
    # A. Uniformly at random among all possible infectors:
    ...
    # B. Probabilistic sampling - sample according to:
    ...
    for (j in 1:howmany.run){
      ...
      # option b:
        (if(maxtime==0){0}else{abs(onset_diff-maxtime)/maxtime}) + (if(maxdist==0){0}else{abs(distance-maxdist)/maxdist}) )
    }
    ...
    if (which.wave==1){
      saveRDS(trees_sam, file = paste0("sampled_trees/", names[i], model, ".rds"))
    }else{saveRDS(trees_sam, file = paste0("sampled_trees/",names[i], model, "_w2.rds"))}
    
    # Plot SIs of 10 of the sampled trees
    if (which.wave==1){
    pdf(paste0("../Figures/obs_si_", names[i],".pdf"), height = 8.3, width= 11.7, paper="a4r")
    }else{pdf(paste0("../Figures/w2_obs_si_", names[i],".pdf"), height = 8.3, width= 11.7, paper="a4r")}
  ...
  if (which.wave==1){
    saveRDS(results_sam1, file = paste0(model, "_results.rds"))
  }else{saveRDS(results_sam1, file = paste0(model, "_w2_results.rds"))}
 ...
 ## Pooled tree version
  ...
  for (i in 1:length(names)){
    ...
    if (which.wave==1){
      trees_sam[((i-1)*howmany.run +1):(i*howmany.run)] <- readRDS(file = paste0("sampled_trees/", names[i], model, ".rds"))[1:howmany.run]
    }else{trees_sam[((i-1)*howmany.run +1):(i*howmany.run)] <- readRDS(file = paste0("sampled_trees/",names[i], model, "_w2.rds"))[1:howmany.run]}
  }
  ...
  if (which.wave==1){
    saveRDS(results_sam_p, file = paste0(model, "_results.rds"))
  }else{saveRDS(results_sam_p, file = paste0(model, "_w2_results.rds"))}
