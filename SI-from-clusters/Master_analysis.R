...
if (which.wave==1){model <- paste0(model, "_w1")}else{model <- paste0(model, "_w2")}
...
if (relabel){
  # Wave 1 = A, wave 2 = B. Numeric order by earliest onset date
  ...
  new.names <- paste0("cluster_", ifelse(which.wave==1, "A", "B"), 1:length(names))[match(sub(".*_", "", names), ord)]
