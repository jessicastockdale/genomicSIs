####################################################################################
### This script creates the 'transmission cloud', for Victorian COVID-19 data wave 2
###
...
fasta <- "w2_genomes.fasta" # This should be the sequences from gisaid, aligned and with names matching gisaid IDs
...
mdt <- mdt %>% dplyr::rename(cluster_id = wave2_cluster_id)
...
min_day_diff <- 1
max_day_diff <- 35
output_filename <- "../data_Victoria/w2_transmission_cloud.csv"

# remove cases with no symptom onset
mdt <- mdt[!is.na(mdt$onset_date),]  

# remove rows in metadata that are not wave 2 sequences
...
write.csv(mdt, "../data_Victoria/epidata_w2clusters.csv")

#print list of case IDs in clusters
dir.create(file.path("../data_Victoria/cluster_ids"), showWarnings = FALSE)
for(i in 1:length(tmp$cluster_id)){ 
  x <- unique(tmp$data[[i]]$GISAID_ID)
  filename <- file.path("../data_Victoria/cluster_ids",paste("cluster_", tmp$cluster_id[[i]],"_IDs.txt",sep=""))
